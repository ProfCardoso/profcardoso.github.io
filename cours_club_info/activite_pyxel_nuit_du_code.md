...
👉 Ces fonctions devront être utilisé dans la fonction `update` de la bibliothèque Pyxel.
...
5. Ecrire une fonction `gerer_joueur` qui affiche votre personnage contrôlé par les flèches. Cette fonction devra être utilisé dans la fonction `update` pour afficher le joueur. N'oubliez pas de "dessiner" votre joueur !
...
- fais apparaître les ennemies (fonction creer_ennemie) 
- déplace les ennemies encore en vie
- supprime les ennemis touchés ou au bout de l'écran.
...
10. Lancer la page de fin !
    
👉 Amélioration possible : ajouter le score s'il est meilleur que celui précédent uniquement.

<details>
  <summary style="cursor: pointer; font-weight: bold;"><u>Voici une possibilité du jeu Dodge&Shoot :</u></summary>
