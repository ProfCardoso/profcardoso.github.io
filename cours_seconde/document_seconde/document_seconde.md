# Documents de Seconde


<link rel="stylesheet" href="../../assets/style.css" />

## Thème 1 : Les réseaux sociaux

<div class="cours-section">
  <div class="boites-lecons">

    <a class="boite" href="./rs_graphe/Reseau_Social.zip">
      <h3>Réseaux Sociaux</h3>
    </a>

    <a class="boite" href="./rs_graphe/Graphe.zip">
      <h3>Graphe</h3>
    </a>

    <a class="boite" href="./rs_graphe/Milgram.zip">
      <h3>Milgram</h3>
    </a>

  </div>
</div>

## Thème 2 : Internet

<div class="cours-section">
  <div class="boites-lecons">

    <a class="boite" href="./internet/activité_internet.pdf">
      <h3>Activité Internet</h3>
    </a>

    <a class="boite" href="./internet/cours_internet.pdf">
      <h3>Cours Internet</h3>
    </a>

    <a class="boite" href="./internet/cours_ip.pdf">
      <h3>Cours Ip</h3>
    </a>

    <a class="boite" href="./internet/exo_ip.pdf">
      <h3>Exercice Ip</h3>
    </a>

    <a class="boite" href="./internet/TCP.pdf">
      <h3>TCP</h3>
    </a>
  </div>
</div>


## Thème 3 : Photographie Numérique 

<div class="cours-section">
  <div class="boites-lecons">

    <a class="boite" href="./photo_num/La_photographie_numerique_Activite_réponse.pdf">
      <h3>Introduction Photographie Numérique</h3>
    </a>

    <a class="boite" href="./photo_num/Cours_Photographie_Numerique.pdf">
      <h3>Cours Photographie Numérique</h3>
    </a>

    <a class="boite" href="./photo_num/CoursPhotosite.pdf">
      <h3>Cours Photosite </h3>
    </a>

  </div>
</div>


## Programmation 

<div class="cours-section">
  <div class="boites-lecons">

    <a class="boite" href="./python/cours_prog.pdf">
      <h3>Introduction Python</h3>
    </a>

    <a class="boite" href="./python/cours_prog_2.pdf">
      <h3>Opérateur Python</h3>
    </a>

    <a class="boite" href="./python/FichePython3.pdf">
      <h3>Boucle Python</h3>
    </a>

    <a class="boite" href="./python/ExerciceBonusFichePython3.pdf">
      <h3>Exercice Boucle Python</h3>
    </a>

    <a class="boite" href="./python/FichePython4.pdf">
      <h3>Exercice Boucle Python</h3>
    </a>

  </div>
</div>